...
def get_note(input_file: Path) -> str:
    results_dir = input_file.parent / "result"
...
if __name__ == "__main__":
    ...
    file_path_name = Path(f"./result/exp3/{assistant_name}/{patient_id}/case.json")
    ...
    with open(Path(f"./result/exp3/{assistant_name}/{patient_id}/note.json"), "w", encoding="utf-8") as f:
        json.dump(note, f, ensure_ascii=False, indent=4)
    rendered_note = render_note(note)
    with open(Path(f"./result/exp3/{assistant_name}/{patient_id}/rendered_note.md"), "w", encoding="utf-8") as f:
        f.write("\n\n---\n\n".join(rendered_note))
